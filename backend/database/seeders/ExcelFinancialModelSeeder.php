...
{
    /**
     * Run the database seeds with exact parameters from financial model Excel spec photos.
     */
    public function run(): void
    {
        // 1. Ensure finance user, company, and project exist
        ...
        // Get company for finance user
        $access = UserCompanyAccess::where('user_id', $financeUser->id)->first();
        if ($access) {
            $company = Company::find($access->company_id);
        } else {
            $company = Company::firstOrCreate(['name' => 'Smartcoop Corp']);
        ...
        // Exact data mapping transcribed from photos (2025 to 2029)
        $modelData = [
            2025 => [
                ...
                'founders_pre_seed_pct' => 100.0,
                'esop_pre_seed_pct' => 0.0,
                'investor_pre_seed_pct' => 0.0,
                'founders_seed_investment' => 0,
                'esop_seed_investment' => 0,
        ...
        foreach ($projects as $project) {
            ...
            $this->command->info("Seeded and recalculated financial model for Project: {$project->name} (ID: {$project->id}) under finance@test.com");
        }
